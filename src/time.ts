...
export type TimeSource = 'network' | 'cached-network' | 'device';

export type ManilaDateInfo = {
  epochMs: number;
  source: TimeSource;
  dateKey: string;
  displayDate: string;
...
export async function getManilaDateInfo(): Promise<ManilaDateInfo> {
  const trusted = await getTrustedEpochMs();
  return buildManilaDateInfo(trusted.epochMs, trusted.source);
}

export function buildManilaDateInfo(epochMs: number, source: TimeSource = 'device'): ManilaDateInfo {
...
export function getMillisecondsUntilNextManilaMidnight(epochMs = Date.now()): number {
...
async function fetchNetworkEpochMs(): Promise<number | null> {
...
function parseEpochMsFromTimeApi(data: Record<string, unknown>): number | null {
  const unixTime = data.unixtime;
  if (typeof unixTime === 'number') return unixTime * 1000;

  const epochMilliseconds = data.epochMilliseconds;
  if (typeof epochMilliseconds === 'number') return epochMilliseconds;
...
function saveTrustedOffset(offsetMs: number): void {
  const payload: SavedOffset = {
    offsetMs,
...
function loadTrustedOffset(): SavedOffset | null {
  try {
    const raw = localStorage.getItem(TRUSTED_TIME_OFFSET_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<SavedOffset>;
    if (typeof parsed.offsetMs !== 'number' || typeof parsed.savedAtEpochMs !== 'number') return null;
...
function getManilaParts(epochMs: number): { year: number; month: number; day: number } {
...
function getNextManilaMidnightEpochMs(year: number, month: number, day: number): number {
